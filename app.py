...
async def generate_jwt_token(uid, password):
    try:
        ...
        url = f"http://87.232.72.68:3005/token?uid={uid}&password={encoded_password}"
...
def add_account():
    ...
    Usage: /add_account?uid=123456&pass=xyz&region=IND&key=NARUTO
    """
...
def reset_cache():
    ...
    if key != "NARUTO":
        return jsonify({"error": "Invalid key"}), 403
    liked_cache.clear()
    return jsonify({"message": "Cache cleared", "credit": "@narutocodexofc"})

if __name__ == '__main__':
    print("🚀 Smart Like API with Account Manager")
    print("📍 Endpoints:")
    print("   GET  /like?uid=UID&server_name=REGION&key=NARUTO")
    print("   GET  /add_account?uid=...&pass=...&region=...&key=NARUTO")
    print("   GET  /reset-cache?key=NARUTO")
